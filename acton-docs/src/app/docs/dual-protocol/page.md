...
| `service.port` | integer | `8080` | Primary HTTP port (also serves gRPC in single-port mode) |
...
[grpc]
enabled = true
use_separate_port = false  # Automatic protocol detection on service.port
```
...
`GrpcServicesBuilder` collects every gRPC service you register and returns a `tonic::service::Routes` value, which `.with_grpc_services()` accepts. Call `.add_service()` once per service — a single `.with_grpc_services()` call carries them all.

The `build()` argument is an optional `AppState`. Pass `None` when you don't enable the health service; pass `Some(state)` when you do, so health checks can inspect your configured dependencies.
...
Enable gRPC reflection for dynamic client discovery. `GrpcServicesBuilder` builds the reflection service for you — call `.with_reflection()` and register the file descriptor set generated by `tonic-build`:
