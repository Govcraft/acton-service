...
Each WebSocket connection gets a UUIDv7-backed `wsconn_` TypeID generated by `mti`. Connection IDs implement `Clone`; clone an ID when passing it to an owner while retaining it for later use. `as_uuid()` and `From<Uuid>` preserve legacy UUID bits.
...
broadcaster.register(connection_id.clone(), sender_channel).await;
...
async fn handle_socket(socket: WebSocket, broadcaster: Arc<Broadcaster>) {
    ...
    broadcaster.register(connection_id.clone(), tx.clone()).await;
