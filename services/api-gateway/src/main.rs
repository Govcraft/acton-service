...
use api_gateway::{Config, AppState, handlers};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    tracing::info!("  POST /users - Create user (proxied to backend)");
    tracing::info!("  GET  /users - List users (proxied to backend)");
    tracing::info!("  GET  /users/:id - Get user (proxied to backend)");
...
fn create_http_router(state: AppState) -> Router {
    ...
    Router::new()
        ...
        .route("/users", get(handlers::list_users_proxy).post(handlers::create_user_proxy))
